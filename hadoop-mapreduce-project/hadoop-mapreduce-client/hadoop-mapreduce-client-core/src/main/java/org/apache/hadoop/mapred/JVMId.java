...
class JVMId {
  ...
  public JVMId(JobID jobId, boolean isMap, long id, int attemptId) {
    this.jvmId = id;
    this.isMap = isMap;
    this.jobId = jobId;
  ...
  public JVMId (String jtIdentifier, int jobId, boolean isMap, long id, int attemptId) {
    this(new JobID(jtIdentifier, jobId), isMap, id, attemptId);
  }
  ...
  @Override
  public int hashCode() {
    // SS_FIXME: Fix this with attemptId
    // Generated by IntelliJ IDEA 13.1.
    int result = (isMap ? 1 : 0);
    result = 31 * result + jobId.hashCode();
    result = 31 * result + (int) (attemptId ^ (attemptId >>> 32));
  ...
  public int compareTo(JVMId that) {
    ...
    if(jobComp == 0) {
      if(this.isMap == that.isMap) {
        int compareValue =  Long.valueOf(this.jvmId).compareTo(that.jvmId);
        if (compareValue == 0) {
          return (Integer.valueOf(this.attemptId).compareTo(that.attemptId));
        } else {
          return compareValue;
        }
      } else {
